const Map<String, String> ruTranslations = {
  'search_hint': 'Найти еду, такси, посылку...',
  'tile_food': 'ЕДА',
  'tile_taxi': 'ТАКСИ',
  'tile_parcels': 'ПОСЫЛКИ',
  'chip_shops': 'Магазины',
  'chip_veggies': 'Овощи и фрукты',
  'chip_supplements': 'БАДы',
  'chip_pharmacy': 'Аптеки',
  'banner_collab': 'Рекомендации от Айтыма Жакупова',
  'aitym_screen_title': 'Рекомендации Айтыма',
  'aitym_badge': 'Автор подборки',
  'back': 'Назад',
  'retry': 'Повторить',
  'loading': 'Загрузка...',
  'error_generic': 'Что-то пошло не так. Попробуйте ещё раз.',
  'hub_default_address': 'Мангилик Ел, 28',
  'currency_tg': 'тг',
  'geocoding_no_results': 'Адреса не найдены',
  'car_class_economy': 'Эконом',
  'car_class_comfort': 'Комфорт',
  'car_class_comfort_plus': 'Комфорт+',
  'car_class_business': 'Бизнес',
  'car_class_premium': 'Премиум',
  'car_class_eco': 'Эко',
  'payment_card': 'Карта',
  'payment_cash': 'Наличные',
  'payment_kaspi_transfer': 'Kaspi перевод',
  'payment_halyk_transfer': 'Halyk перевод',
  'taxi_from': 'Откуда',
  'taxi_to': 'Куда',
  'ride_status_searching': 'Ищем водителя',
  'ride_status_assigned': 'Водитель найден',
  'ride_status_heading': 'Водитель едет к вам',
  'ride_status_arrived': 'Водитель на месте',
  'ride_status_in_progress': 'В пути',
  'ride_status_completed': 'Поездка завершена',
  'ride_passenger_exit_button': 'Выхожу',
  'taxi_from_title': 'Откуда поедем?',
  'taxi_to_title': 'Куда едем?',
  'address_confirm_button': 'Подтвердить',
  'taxi_order_button': 'Заказать',
  'taxi_fill_addresses': 'Укажите адреса',
  'taxi_choose_payment': 'Способ оплаты',
  'taxi_driver': 'Водитель',
  'taxi_finish_test': 'Завершить (тест)',
  'taxi_searching_driver': 'Ищем водителя',
  'taxi_searching_driver_subtitle': 'Обычно занимает 1-3 минуты',
  'taxi_cancel_search': 'Отменить поиск',
  'taxi_driver_found': 'Водитель найден',
  'taxi_arrives_in': 'Подъедет через',
  'taxi_min_short': 'мин',
  'taxi_call_driver': 'Позвонить',
  'taxi_rate_trip_title': 'Оцените поездку',
  'taxi_rate_comment_hint': 'Комментарий (необязательно)',
  'taxi_rate_submit': 'Отправить',
  'cat_food': 'Еда',
  'cat_taxi': 'Такси',
  'cat_parcel': 'Посылки',
  'cat_shops': 'Магазины',
  'cat_veggies': 'Овощи',
  'cat_supplements': 'БАДы',
  'cat_pharmacy': 'Аптека',
  'tab_home': 'Главная',
'tab_profile': 'Профиль',
'tab_history': 'История',
  'nav_2gis': '2ГИС',
  'nav_yandex': 'Яндекс Навигатор',
  'location_disabled': 'Геолокация выключена на устройстве',
  'location_denied': 'Доступ к геолокации отклонён',
  'location_denied_forever': 'Доступ заблокирован навсегда — включите в настройках',
  'courier_go_online': 'Выйти на смену',
  'courier_searching_orders': 'Ищем заказы для вас',
  'courier_new_order': 'Новый заказ',
  'courier_swipe_to_accept': 'Свайпните, чтобы принять',
  'courier_status_to_restaurant': 'Еду в ресторан',
  'courier_status_picked_up': 'Забрал заказ',
  'courier_status_delivering': 'Везу клиенту',
  'courier_status_delivered': 'Доставлено',
  'courier_meters_left': 'м до точки',
  'courier_sound_settings_title': 'Звук уведомлений',
  'courier_sound_option_1': 'Классический',
  'courier_sound_option_2': 'Мягкий',
  'courier_sound_option_3': 'Бодрый',
  'courier_sound_play': 'Прослушать',
  'courier_sound_select': 'Выбрать',
  'driver_searching_order': 'Ищем заказ...',
  'driver_new_order': 'Новый заказ',
  'driver_accept': 'Принять',
  'driver_decline': 'Отклонить',
  'driver_categories_title': 'Активные категории',
  'driver_category_taxi': 'Такси',
  'driver_category_food': 'Еда',
  'driver_category_parcels': 'Посылки',
  'restaurant_orders_title': 'Заказы',
  'restaurant_status_new': 'Новый',
  'restaurant_status_accepted': 'Принят',
  'restaurant_status_cooking': 'Готовится',
  'restaurant_status_ready': 'Готово',
  'restaurant_status_given_to_courier': 'Передан курьеру',
  'restaurant_accept_order': 'Принять',
  'restaurant_mark_cooking': 'Готовим',
  'restaurant_mark_ready': 'Готово',
  'restaurant_mark_given': 'Передать курьеру',
  'food_restaurants_title': 'Рестораны',
  'food_min_order': 'мин. заказ',
  'food_delivery_time': 'мин',
  'food_free_delivery': 'Бесплатная доставка',
  'food_menu_search_hint': 'Найти блюдо...',
  'food_add_to_cart': 'В корзину',
  'food_cart_title': 'Корзина',
  'food_cart_empty': 'Корзина пуста',
  'food_go_to_checkout': 'Оформить заказ',
  'food_checkout_title': 'Оформление',
  'food_checkout_address': 'Адрес доставки',
  'food_checkout_time': 'Время доставки',
  'food_checkout_payment': 'Способ оплаты',
  'food_checkout_confirm': 'Подтвердить заказ',
  'food_service_fee': 'Сервисный сбор',
  'food_total': 'Итого',
  'parcel_title': 'Отправить посылку',
  'parcel_category_title': 'Категория посылки',
  'parcel_category_up_to_5kg': 'До 5 кг',
  'parcel_category_5_20kg': '5–20 кг',
  'parcel_category_30kg_plus': '30 кг и больше',
  'parcel_delivery_type_title': 'Тип доставки',
  'parcel_delivery_to_address': 'До адреса',
  'parcel_delivery_door_to_door': 'От двери до двери',
  'parcel_sender_title': 'Отправитель',
  'parcel_receiver_title': 'Получатель',
  'parcel_name_hint': 'Имя',
  'parcel_phone_hint': 'Телефон',
  'parcel_entrance_hint': 'Подъезд',
  'parcel_floor_hint': 'Этаж',
  'parcel_apartment_hint': 'Квартира',
  'parcel_comment_hint': 'Комментарий курьеру (необязательно)',
  'parcel_order_button': 'Заказать',
  'auth_login_title': 'Вход',
  'auth_register_title': 'Регистрация',
  'auth_name_hint': 'Ваше имя',
  'auth_phone_hint': 'Номер телефона',
  'auth_password_hint': 'Пароль',
  'auth_login_button': 'Войти',
  'auth_register_button': 'Зарегистрироваться',
  'auth_google_button': 'Продолжить с Google',
  'auth_invalid_credentials': 'Неверный номер или пароль',
  'auth_register_error': 'Ошибка регистрации. Попробуйте ещё раз.',
  'auth_name_required': 'Введите ваше имя',
  'auth_phone_required': 'Введите номер телефона',
  'auth_password_required': 'Введите пароль',
  'profile_title': 'Профиль',
  'profile_edit': 'Редактировать',
  'profile_section_account': 'Аккаунт',
  'profile_section_addresses': 'Адреса',
  'profile_section_payment': 'Оплата',
  'profile_section_history': 'История',
  'profile_section_settings': 'Настройки',
  'profile_section_support': 'Поддержка',
  'profile_logout': 'Выйти',
  'profile_city_label': 'Город',
  'profile_role_client': 'Клиент',
'profile_role_courier': 'Курьер',
'profile_role_driver': 'Водитель',
'profile_role_restaurant': 'Ресторан',
'profile_role_admin': 'Администратор',
'profile_become_partner_title': 'Стать партнером Вентал',
'profile_become_partner_subtitle': 'Подключайте водителей и зарабатывайте',
  'city_picker_title': 'Выберите город',
  'payment_methods_title': 'Способ оплаты',
  'payment_methods_cards': 'Карты',
  'payment_add_card': 'Добавить карту',
  'price_breakdown_title': 'Из чего складывается цена',
  'price_breakdown_boarding': 'Посадка',
  'price_breakdown_distance': 'Расстояние',
  'price_breakdown_total': 'Итого',
  'order_success_button': 'На главную',
  'parcel_success_title': 'Посылка отправлена!',
  'parcel_success_subtitle': 'Курьер уже в пути к отправителю',
  'food_success_title': 'Заказ оформлен!',
  'food_success_subtitle': 'Ресторан уже готовит ваш заказ',
  'parcel_confirm_button': 'Подтвердить за',
  'parcel_delivery_building_to_building': 'Здание — здание',
  'parcel_intercom_hint': 'Домофон/пароль',
  'parcel_transport_walking': 'Пеший',
  'parcel_transport_scooter': 'Скутер',
  'parcel_transport_car': 'Машина',
  'parcel_transport_truck': 'Грузовая',
  'parcel_weight_up_to_5kg': 'до 5 кг',
  'parcel_weight_5_15kg': '5–15 кг',
  'parcel_weight_15_25kg': '15–25 кг',
  'parcel_weight_25kg_plus': '25+ кг',
  'parcel_weight_up_to_15kg': 'до 15 кг',
  'parcel_weight_up_to_25kg': 'до 25 кг',
  'parcel_address_hint': 'Адрес',
  'parcel_details_title': 'Детали заказа',
  'parcel_details_subtitle': 'Что в посылке, комментарий курьеру, хрупкий груз и способ оплаты',
  'parcel_details_filled': 'Детали заполнены',
  'parcel_whats_inside': 'Что в посылке?',
  'parcel_whats_inside_hint': 'Опишите, что в посылке',
  'parcel_comment_example': 'Например: позвонить перед доставкой',
  'parcel_photo_title': 'Фото посылки',
  'parcel_fragile_title': 'Хрупкий груз',
  'parcel_fragile_subtitle': 'Требует бережного обращения',
  'parcel_next_button': 'Далее',
  'parcel_delivery_selection_title': 'Выбор доставки',
  'parcel_how_to_deliver': 'Как доставить?',
  'parcel_weight_title': 'Вес посылки',
  'parcel_estimated_price': 'Ориентировочная стоимость',
  'parcel_continue_button': 'Продолжить',
  'parcel_cost_title': 'Стоимость',
  'tab_services': 'Сервисы',
'main_hub_services_title': 'Сервисы',
'main_hub_see_all': 'Все',
  'services_title': 'Сервисы',
  'history_active_tab': 'Активные',
'history_past_tab': 'История',
'history_active_title': 'Активные',
'history_order_in_transit': 'Заказ в пути',
'history_delivery_eta': 'Доставка • 25–30 мин',
'history_courier_picked_up': 'Курьер забрал заказ',
'history_filter_all': 'Все',
'history_filter_delivery': 'Доставка',
'history_composition': 'Состав',
'history_repeat_order': 'Повторить заказ',
};